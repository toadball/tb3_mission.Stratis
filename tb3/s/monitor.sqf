...
EStartNum = EAST countSide allUnits; EArrayNum = count(EArray);
RStartNum = RESISTANCE countSide allUnits; RArrayNum = count(RArray);
...
while { true } do {
	...
	if ( EStartNum > 0) then { ECasNum = EStartNum - (EAST countSide allUnits); };
	if ( WStartNum > 0) then { WCasNum = WStartNum - (WEST countSide allUnits); };
	if ( RStartNum > 0) then { RCasNum = RStartNum - (RESISTANCE countSide allUnits); };
